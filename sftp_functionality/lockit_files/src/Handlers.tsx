...
export const createHandlers = (
  ...
  handleConnect: async () => {
    try {
      ...
      console.log('Making connection request:', `${BASE_URL}/connect`);
...
handleDownload: async (file: FileItem) => {
  try {
    ...
    // Define paths - ONLY use the Downloads folder
...
// Add these methods to the handlers object in Handlers.tsx
...
handleMoveTo: async (file: FileItem) => {
  ...
  confirmRename: async () => {
    if (!state.selectedFile || !state.newFileName) return;
...
// Add this to your handlers object in Handlers.tsx
...
// In Handlers.tsx, update both functions:
  ...
  try {
    ...
    if (imageTypes.includes(fileType || '')) {
      ...
      try {
        ...
        const checkResponse = await fetch(`${BASE_URL}/open`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
